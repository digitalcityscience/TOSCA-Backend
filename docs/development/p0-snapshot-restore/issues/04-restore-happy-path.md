...
- [ ] **Preflight:** `manifest.json` exists and artifact sha256s match (reuse the 03 verify → **abort
      ...
      mismatch = second lost-Shapefile risk). Interactive "this is destructive, `<id>` will be
      restored" confirm, skipped by `--yes` / `YES=1`.
- [ ] **Pre-restore safety snapshot (Q9):** run `create` with `LABEL=pre-restore-safety`,
      `kind=safety` in its manifest. **If it fails, abort the restore** — never proceed without the
      safety net.
- [ ] **Quiesce:** stop `django geoserver` (+`web nginx` on prod). `db` stays up.
- [ ] **Postgres restore (fresh-DB method, avoids `--clean` cascade fragility):**
      - Via maintenance db `postgres`: terminate other connections to `$PG_DATABASE`, then
        `DROP DATABASE IF EXISTS` + `CREATE DATABASE … OWNER $PG_SUPERUSER` (`ON_ERROR_STOP=1`).
        Global roles live in the volume — `DROP DATABASE` does not remove them, and the pg_data
        volume is **never** deleted.
      - `pg_restore -U $PG_SUPERUSER -d $PG_DATABASE < backups/<id>/postgres.dump`; log errors rather
        than `--exit-on-error` (PostGIS extension notices are normal).
- [ ] **GeoServer volume restore** (geoserver stopped): via `--volumes-from $GS_CID`, wipe
      `/geoserver_data/data/*` then `tar xzf` the archive into it.
- [ ] **Restart in correct order:** `start geoserver` → wait healthy (`/geoserver/web`) →
      `start django` → wait healthy (`/readyz`) → `start web nginx` (prod).
- [ ] **Post-restore verify:** run `geoengine_smoke_test`. On failure print

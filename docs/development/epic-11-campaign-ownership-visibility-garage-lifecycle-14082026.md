...
**Durum:** PR1/PR2/PR3 implementasyonu tamamlandı (2026-08-14). Ticket kapsamı bitti.
...
3. **PR3 — Campaign/GeoStory archive & restore lifecycle** ✅ (2026-08-14, implementasyona başlandı)
   ...
   - ✅ `core/media_lifecycle.py::MediaLifecycleService` — `desired_alias_for_asset()` öncelik sırası: (1) Campaign archived → `media_archive`, (2) sahibi GeoStory archived (Campaign aktif olsa bile) → `media_archive`, (3) aksi halde `Campaign.visibility`'e göre `media_public`/`default`. `move_one()` PR2'nin `MediaPathMigrator` ile aynı copy→verify→DB update→delete sırasını izliyor (kesintiye dayanıklı, aynı-boyut resume). Hero-image eşleşen asset'ler bilinçli olarak atlanıyor (`ACTION_SKIPPED_HERO_IMAGE`) — `GeoStory.hero_image` `default` storage'a sabit bir `ImageField`, ayrı bir alias'a taşımak `hero_image.url`'i kırar; bu bilinen bir kapsam sınırı (§4'te detaylandırıldı).
...
- PR3: ✅ lifecycle state-transition testleri — `core/tests/test_media_lifecycle.py` (13 test: `desired_alias_for_asset` önceliği [orphan→None, campaign archived→archive, story archived ama campaign aktif→archive, visibility→public/default], `move_one` copy→verify→DB update→delete + resume + hero-image skip + missing-source fail, `sync_campaign_assets`/`sync_story_assets` sweep kapsamı), signal testleri — `core/tests/test_media_lifecycle_signals.py` (7 test: create'te tetiklenmiyor, ilgisiz alan değişikliğinde tetiklenmiyor, status/visibility değişikliğinde tetikleniyor, Campaign ve GeoStory için ayrı ayrı — `GeoServerSecuritySyncService`/`organizations.signals` testleriyle aynı mock-the-service deseni), backfill migration testi — `core/tests/test_storage_alias_backfill.py` (mevcut EditorJS-prefix'li satırların `media_public`'e taşınması). Tüm suite (1105 test, 3 önceden var olan garage-bağımlı editorjs-upload testi hariç) yeşil.

...
Following claude.md: Use geoserver-rest library but in controlled adapter
"""
import sys
import os
import logging
...
class GeoServerClient:
    ...
    def create_workspace(self, name: str) -> Dict:
        ...
        try:
            # 1️⃣ PRE-CHECK: Workspace var mı?
            ...
            if not verification.get('verified', False):
                ...
                # Burada rollback logic eklenebilir
                
            # 5️⃣ FINAL RESPONSE: Tüm bilgileri topla
            final_result = {
                'success': True,
                'workspace': name,
    ...
    def create_postgis_store(
        ...
        """
        try:
            result = self._client.create_featurestore(
                store_name=name,
                workspace=workspace,
                db=database,
                host=host,
    ...
    def publish_featuretype(
        ...
        try:
            ...
            wfs_url = f"{base_url}/ows?service=WFS&version=1.0.0&request=GetFeature&typeName={workspace}:{layer_name}"
            wms_url = f"{base_url}/{workspace}/wms?service=WMS&version=1.1.0&request=GetMap&layers={workspace}:{layer_name}"
    ...
    def delete_layer(self, workspace: str, layer_name: str) -> Dict:
        ...
        """
        try:
            result = self._client.delete_layer(layer_name, workspace)
    ...
    def workspace_exists(self, workspace: str) -> bool:
        """
        Check if workspace exists in GeoServer - PRE-CHECK implementation
    ...
    def store_exists(self, workspace: str, store_name: str) -> bool:
        """
        Check if datastore exists in GeoServer - PRE-CHECK implementation
    ...
    def validate_response(self, response, operation: str) -> Dict:
        """
        Validate GeoServer REST response - RESPONSE VALIDATION implementation
        ...
        try:
            ...
            # If response is already a dict with success info, return it
            ...
            # If response is a string or other format, wrap it
            return {
                'success': True,
    ...
    def post_verify_workspace(self, workspace_name: str, expected_exists: bool = True) -> Dict:
        """
        Post-operation verification for workspace - POST-CHECK implementation
    ...
    def create_store(self, workspace: str, store_data: dict) -> Dict:
        ...
        try:
            # 1️⃣ PRE-CHECK: Store zaten var mı?
            ...
            if not validated_result.get('success', False):
                raise GeoServerPublishError(f"Store creation failed validation: {validated_result.get('message')}")
            
            # 4️⃣ POST-CHECK: Store gerçekten oluştu mu?
    ...
    def pre_check_store(self, workspace: str, store_name: str) -> Dict:
        ...
        try:
            stores = self._client.get_datastores(workspace)
            store_exists = store_name in [store['name'] for store in stores.get('dataStores', {}).get('dataStore', [])]
            return {
                'success': True,
                'exists': store_exists,
    ...
    def post_verify_store(self, workspace: str, store_name: str, expected_exists: bool = True) -> Dict:
        ...
        try:
            stores = self._client.get_datastores(workspace)
            actual_exists = store_name in [store['name'] for store in stores.get('dataStores', {}).get('dataStore', [])]
